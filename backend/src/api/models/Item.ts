...
class Item {
  ...
  constructor(
    ...
    price: number,
    unitAmount: number,
    arbitraryUOM: string // Pass the arbitrary unit of measurement string
  ) {
    this.name = name;
    ...
    const { normalizedUOM, scaledUnitAmount } = this.convertAndNormalizeUOM(
      arbitraryUOM,
      unitAmount
    );
    this.unitOfMeasurement = normalizedUOM;
    this.unitAmount = scaledUnitAmount;
    this.normalizedPrice = this.calculateNormalizedPrice(
      price,
      this.unitAmount,
  ...
  private convertAndNormalizeUOM(
    arbitraryUOM: string,
    unitAmount: number
  ): { normalizedUOM: UnitOfMeasurement; scaledUnitAmount: number } {
    console.log("UOM: ", arbitraryUOM, "\nUnit amount: ", unitAmount);
    let normalizedUOM: UnitOfMeasurement;
    ...
    switch (arbitraryUOM.toLowerCase()) {
      ...
      default:
        console.error(
          "Unsupported unit of measurement defaulting to EACH:",
          arbitraryUOM
        );
        normalizedUOM = UnitOfMeasurement.EACH;
        conversionFactor = 1; // Keeping it 1 for consistency
  ...
  private calculateNormalizedPrice(
    price: number,
    unitAmount: number,
    ...
    switch (uom) {
      ...
      default:
        console.error("Invalid unit of measurement for normalization: ", uom);
        return price;
    }
  }
}
